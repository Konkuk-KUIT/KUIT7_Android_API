...
class PostViewModel @Inject constructor(
    ...
    fun createPost() {
        viewModelScope.launch {
            ...
            runCatching {
                ...
                _eventFlow.emit(PostEvent.ShowSnackbar("Post created!"))
        ...
        viewModelScope.launch {
            ...
            runCatching {
                postRepository.updatePost(
                    id = postId.toInt(),
                    title = _postEditFormState.value.title,
                    body = _postEditFormState.value.body,
                    userId = _postEditFormState.value.userId
                )
            }.onSuccess { post ->
                ...
                addOrReplacePostInList(post)
                _eventFlow.emit(PostEvent.ShowSnackbar("Post updated!"))
                _eventFlow.emit(PostEvent.NavigateBack)
            }.onFailure { error ->
    ...
    fun deletePost(postId: Long) {
        viewModelScope.launch {
            runCatching {
                ...
                _eventFlow.emit(PostEvent.ShowSnackbar("Post deleted."))
                _eventFlow.emit(PostEvent.NavigateBack)
            }.onFailure { error ->
    ...
    fun onUpdateAuthor(author: String) {
        _postCreateFormState.value =
            _postCreateFormState.value.copy(author = author)
    }
