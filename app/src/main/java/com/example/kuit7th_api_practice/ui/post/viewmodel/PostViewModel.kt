...
class PostViewModel @Inject constructor(
    ...
    //앱 안에서만 작성/수정/삭제 결과를 유지하기 위한 로컬 저장소
    private val createdPosts = mutableMapOf<Int, Post>()//<키,벨류>로 게시글 저장
    ...
    fun getPostDetail(postId: Long) {
        ...
        viewModelScope.launch {
            ...
            postRepository.getPost(postId.toInt())
                .catch{error->
                    _postDetailUiState.value= PostDetailUiState.Error(error.message ?:"Failed to load post.")
                    _postEditUiState.value= PostEditUiState.Error(error.message ?:"Failed to load post.")
                }
                .collect{ post->
                    showPostDetail(post)
                }
    ...
    fun updatePost(postId: Long) {
        viewModelScope.launch {
            ...
            runCatching {
                ...
                val updatedPost = post.copy(id = postId.toInt())

                if (createdPosts.containsKey(updatedPost.id)) {
                    createdPosts[updatedPost.id] = updatedPost
                } else {
                    updatedPosts[updatedPost.id] = updatedPost
                }
    ...
    private fun applyLocalChanges(posts: List<Post>, userId: Int?): List<Post> {
        val serverPosts = posts
            .filterNot { deletedPostIds.contains(it.id) }//삭제된 게시글 목록에서 뺴기
            .map { post -> updatedPosts[post.id] ?: post }//수정본 있으면 수정본 사용, 없으면 서버에서 받은 원본 사용
